...
enough for that. In this chapter we therefore get to know the data structures
...
points in time. We collect these values in a **list**, recognisable by the
square brackets.
...
the value `22.4`. The index `-1` points to the last element (`23.7`, the
...
not recognisable to outsiders and is error-prone as soon as the order
changes. The dictionary makes the meaning of each value immediately visible
via the key.
````
...
value is returned to the calling location. Once the function is defined, we
...
would be lost, since it is not returned to the calling location.
````
