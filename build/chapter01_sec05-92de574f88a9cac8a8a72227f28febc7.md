...
for celsius in range(0, 101, 20):
    ...
    print(f'{celsius} degrees Celsius correspond to {fahrenheit:.1f} degrees Fahrenheit')
```
...
```
The temperature drops in uniform steps from 180 to 12 degrees Celsius. From
time step 14 onwards it is 12 degrees Celsius, below the mark of 20 degrees
for the first time; the additional message appears there. With `range(15)` we
cover all time steps until the mark is undershot.
````
