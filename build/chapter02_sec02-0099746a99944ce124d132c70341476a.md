...
and functions such as `np.exp()` and summarised them into key figures with
...
that the generator delivers, and then characterise the test run
statistically.
````
...
and a half time constants, about 97 per cent of this limit value is reached.
````
...
```{admonition} Part 5: Characterise the test run
:class: tip
