...
incidentally integer values 0, 1, 2 and 3 as floats, recognisable by the
...
value, but in a summarising measure: how large is a measured value on
...
With these four functions, every measurement series can be characterised at a
...
With `np.mean()`, `np.std()`, `np.min()` and `np.max()` we summarise a
measurement series in a few key figures.

In the next chapter we apply these tools in a coherent project and analyse
